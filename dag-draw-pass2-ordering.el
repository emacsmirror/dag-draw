...
(defun dag-draw--count-crossings (graph rank1-idx rank2-idx)
  "Count crossings between two ranks specified by indices.
Convenience wrapper around dag-draw--count-crossings-between-ranks."
  (let ((ranks (dag-draw--organize-by-ranks graph)))
    (if (or (>= rank1-idx (length ranks))
            (>= rank2-idx (length ranks))
            (< rank1-idx 0)
            (< rank2-idx 0))
        0  ; Invalid indices
      (dag-draw--count-crossings-between-ranks 
       graph 
       (aref ranks rank1-idx) 
       (aref ranks rank2-idx)))))

(defun dag-draw--optimize-two-layer-crossings (graph rank1-idx rank2-idx)
  "Optimize crossings between two specific layers using GKNV weighted median heuristic.
This implements the core two-layer optimization from GKNV Figure 3-2.
Returns hash table with optimization results."
  (let ((result (ht-create)))
    
    ;; First validate that we have proper rank assignments 
    ;; The paper assumes nodes have been ranked in Pass 1
    (let* ((nodes-list (ht-values (dag-draw-graph-nodes graph)))
           (ranked-nodes (cl-remove-if-not (lambda (n) (dag-draw-node-rank n)) nodes-list)))
      
      (if (< (length ranked-nodes) 2)
          ;; Not enough ranked nodes for two-layer optimization
          (progn
            (ht-set! result 'optimization-applied nil)
            (ht-set! result 'final-crossings 0)
            (message "Two-layer optimization: insufficient ranked nodes (%d)" (length ranked-nodes)))
        
        ;; Proceed with GKNV algorithm: organize nodes by ranks
        (let* ((ranks (dag-draw--organize-by-ranks graph))
               (max-rank (1- (length ranks))))
          
          (if (or (< rank1-idx 0) (< rank2-idx 0)
                  (> rank1-idx max-rank) (> rank2-idx max-rank))
              ;; Invalid rank indices  
              (progn
                (ht-set! result 'optimization-applied nil)
                (ht-set! result 'final-crossings 0)
                (message "Two-layer optimization: invalid ranks %d,%d (max: %d)" 
                         rank1-idx rank2-idx max-rank))
            
            ;; Apply GKNV weighted median heuristic between the two layers
            (let* ((rank1-nodes (aref ranks rank1-idx))
                   (rank2-nodes (aref ranks rank2-idx))
                   (initial-crossings (dag-draw--count-crossings-between-ranks 
                                      graph rank1-nodes rank2-nodes)))
              
              ;; Apply weighted median to rank2 based on rank1 (GKNV Fig 3-2)
              (when (> (length rank2-nodes) 1)
                (let ((reordered-rank2 (dag-draw--order-rank-by-median
                                       graph rank2-nodes rank1-nodes 'down)))
                  (setf (aref ranks rank2-idx) reordered-rank2)))
              
              ;; Apply transposition heuristic (GKNV Fig 3-3)
              (when (> (length (aref ranks rank2-idx)) 1)
                (dag-draw--transpose-adjacent graph ranks rank2-idx))
              
              ;; Calculate final results
              (let ((final-crossings (dag-draw--count-crossings-between-ranks
                                     graph (aref ranks rank1-idx) (aref ranks rank2-idx))))
                ...
                (ht-set! result 'improvement (- initial-crossings final-crossings))
                
                (message "Two-layer GKNV: ranks %d→%d, crossings %d→%d (-%d)" 
                         rank1-idx rank2-idx initial-crossings final-crossings
                         (- initial-crossings final-crossings))))))))
    
    result))

(defun dag-draw--calculate-weighted-median (graph node-id)
  "Calculate weighted median position for a node based on its edges.
Implements the weighted median heuristic from the GKNV paper."
  ;; Find which rank this node is in
  (let* ((node (dag-draw-get-node graph node-id))
         (node-rank (dag-draw-node-rank node))
         (ranks (dag-draw--organize-by-ranks graph))
         (adjacent-rank-idx (if (> node-rank 0) (1- node-rank) (1+ node-rank))))
    
    (if (or (null node-rank) 
            (< adjacent-rank-idx 0)
            (>= adjacent-rank-idx (length ranks)))
        ;; No adjacent rank or invalid rank
        0.0
      
      ;; Calculate median based on adjacent rank
      (let ((adjacent-rank-nodes (aref ranks adjacent-rank-idx)))
        (dag-draw--calculate-median-position graph node-id adjacent-rank-nodes)))))

(defun dag-draw--iterative-crossing-reduction (graph)
  "Apply iterative crossing reduction using the full GKNV algorithm.
Returns hash table with convergence information."
  ;; Use the existing sophisticated convergence detection
  (let* ((ranks (dag-draw--organize-by-ranks graph))
         (convergence-result (dag-draw--crossing-reduction-with-convergence graph ranks)))
    
    ;; Apply the best ordering back to the graph
    (dag-draw--apply-ordering-to-graph graph (ht-get convergence-result 'best-ranks))
