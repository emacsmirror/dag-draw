;;; dag-draw-ascii-edges.el --- ASCII edge drawing for dag-draw -*- lexical-binding: t -*-;; Copyright (C) 2024;; This program is free software; you can redistribute it and/or modify
    ...
    (dotimes (dy (1+ (* 2 search-radius)))
      (dotimes (dx (1+ (* 2 search-radius)))
        (let* ((check-x (+ target-x (- dx search-radius)))
               ...
               (distance (+ (abs (- check-x target-x)) (abs (- check-y target-y)))))          ;; Only check within grid bounds and search radius
          (when (and (>= check-x 0) (< check-x grid-width)
                     (>= check-y 0) (< check-y grid-height)
                     (<= distance search-radius))            (let ((char-at-pos (aref (aref grid check-y) check-x)))
  ...
  (let* ((grid-height (length grid))
    ...
    (dotimes (dy (1+ (* 2 search-radius)))
      (dotimes (dx (1+ (* 2 search-radius)))
        (let* ((check-x (+ target-x (- dx search-radius)))
               (check-y (+ target-y (- dy search-radius)))
               (distance (+ (abs (- check-x target-x)) (abs (- check-y target-y)))))          (when (and (>= check-x 0) (< check-x grid-width)
                     (>= check-y 0) (< check-y grid-height)
                     (<= distance search-radius))            (let ((char-at-pos (aref (aref grid check-y) check-x)))
              (when (memq char-at-pos any-boundary-chars)
                (when (< distance best-distance)
                  (setq best-pos (list check-x check-y))
                  (setq best-distance distance))))))))    ;; If we found a boundary, place arrow adjacent to it
    (when best-pos
