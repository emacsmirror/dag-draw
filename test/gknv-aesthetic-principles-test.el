    ...
    (it "should minimize edge crossings in ASCII output"
      (let ((graph (dag-draw-create-graph)))
        ...
        (let ((ascii-output (dag-draw-render-ascii graph)))
          ;; Should produce clean output with proper edge routing
    ...
    (it "should optimize coordinate assignment for edge straightness"
      (let ((graph (dag-draw-create-graph)))
        ...
        (let ((x-top (dag-draw-node-x-coord (dag-draw-get-node graph 'top)))
          ...
          ;; Should be approximately aligned (some tolerance for centering)
          (expect (abs (- x-top x-center)) :to-be-less-than 5)
          (expect (abs (- x-center x-bottom)) :to-be-less-than 5))))
      ...
      (let ((graph (dag-draw-create-graph)))
        ...
        (let ((x-top (dag-draw-node-x-coord (dag-draw-get-node graph 'top)))
          ...
          (let ((center-x (/ (+ x-left x-right) 2)))
            (expect (abs (- x-top center-x)) :to-be-less-than 10)
            (expect (abs (- x-bottom center-x)) :to-be-less-than 10))))
