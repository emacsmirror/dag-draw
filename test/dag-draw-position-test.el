...
(describe "dag-draw-position"
  
  (describe "Y-coordinate assignment"
  ...
  (describe "X-coordinate assignment"
  ...
  (describe "auxiliary graph creation"
    ...
    (it "should handle complex graph with multiple branches"
      (let ((graph (dag-draw-create-graph)))
        ...
        (let ((y-left (dag-draw-node-y-coord (dag-draw-get-node graph 'left)))
          ...
          (expect x-left :not :to-equal x-right))))  ; Different X positions

  (describe "omega factor calculation"
